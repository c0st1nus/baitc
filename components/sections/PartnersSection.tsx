  ...
  return (
    <section id="partners" className="py-[clamp(5rem,10vw,10rem)]">
      <div className="max-w-[1280px] mx-auto px-[clamp(1.5rem,5vw,4rem)]">
        <motion.div {...fadeUp} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} className="text-center mb-16">
        ...
        <div className="space-y-12">
          {categories.map((cat, ci) => (
            <motion.div
              key={ci}
              {...fadeUp}
              transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1], delay: ci * 0.1 }}
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {cat.logos.map((logo, li) => (
                  ...
                  >
                    <span className="text-sm font-heading font-medium text-center leading-tight">{logo}</span>
                  </div>
                ))}
              </div>
            </motion.div>
        ...
        >
          <a href="#contact" className="text-lime-500 hover:text-lime-400 transition-colors text-sm">
            {t(cta)}
          </a>
        </motion.div>
      </div>
