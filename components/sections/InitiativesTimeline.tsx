...
export function InitiativesTimeline() {
  ...
  return (
    <section id="initiatives" className="py-20 md:py-32 relative overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <SmartText text={t(headline)} as="h2" className="text-h2 text-white mt-4 font-heading mb-6" />
                        ...
                        <div key={`${item.title[0]}-${index}`} className="relative">
                            ...
                            <div className={cn(
                                ...
                                <motion.div 
                                    ...
                                    className={cn("w-full md:w-[calc(50%-40px)]", isRight ? "md:text-left" : "md:text-right")}
                                >
                                    <div className="group relative rounded-2xl overflow-hidden border border-white/10 bg-white/5 aspect-video md:aspect-4/3 lg:aspect-video shadow-2xl">
                                        ...
                                        <div className="absolute top-4 left-4 flex gap-2">
                                             <span className={cn(
                                                ...
                                                item.type === 'blockchain' ? 'text-lime-400 border-lime-500/30' : 'text-cyan-400 border-cyan-500/30'
                                              )}>
                                                {item.type}
                                              </span>
                                ...
                                <motion.div 
                                    ...
                                    className={cn("w-full md:w-[calc(50%-40px)] flex flex-col justify-center", !isRight ? "md:text-right md:items-end" : "md:text-left md:items-start")}
                                >
                                    <h3 className="text-2xl md:text-3xl font-bold text-white mb-3 font-heading leading-tight">{t(item.title)}</h3>
                                    <p className="text-lime-400 text-sm md:text-base font-medium mb-4">{t(item.tagline)}</p>
                                    <p className="text-slate-400 text-sm leading-relaxed mb-6 max-w-lg">
                                        {t(item.description)}
                                    </p>

                                    {/* Stats */}
                                    <div className={cn("grid grid-cols-2 gap-4 mb-6 w-full max-w-md", !isRight ? "md:ml-auto" : "")}>
                                        {item.stats.map((stat, idx) => (
                                            <div key={idx} className={cn("bg-white/5 border border-white/5 rounded-xl p-3 backdrop-blur-sm", !isRight ? "text-right" : "text-left")}>
                                                <div className="text-xl md:text-2xl font-mono font-bold text-white mb-1">{stat.value}</div>
                                                <div className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">{t(stat.label)}</div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    {item.partners && item.partners.length > 0 && (
                                        <div className={cn("flex flex-wrap gap-2 items-center", !isRight ? "md:justify-end" : "md:justify-start")}>
                                            <span className="text-xs text-slate-600 font-mono uppercase mr-2">{t(['Partners:', 'Партнеры:', 'Серіктестер:'])}</span>
                                            {item.partners.map((p, pIdx) => (
                                                <span key={pIdx} className="text-xs text-slate-400 bg-white/5 px-2 py-1 rounded border border-white/5 hover:bg-white/10 transition-colors cursor-default">
                                                    {p}
                                                </span>
                                            ))}
                                        </div>
