  ...
  return (
    <motion.div
      ...
      transition={{
        duration: 0.5,
        ease: [0.16, 1, 0.3, 1],
        delay: side === "ai" ? 0.15 : 0,
      }}
      onAnimationComplete={() => setIsInteractive(true)}
      onMouseEnter={() => onHover(side)}
      onMouseLeave={() => onHover(null)}
      className={cn(
        "glass rounded-2xl p-8 flex flex-col gap-6 cursor-default",
...
export function EcosystemPillars() {
  ...
  const [hoveredMode, setHoveredMode] = useState<"blockchain" | "ai" | null>(
    null,
  );
