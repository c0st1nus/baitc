...
export function ContactSection() {
  const { t } = useLang()
  const { label, headline, description, inquiryTypes, info } = siteConfig.contact
      ...
      <div className="max-w-[1280px] mx-auto px-[clamp(1.5rem,5vw,4rem)]">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <motion.div {...fadeUp} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <SmartText text={t(description)} as="p" className="text-slate-400 text-sm mt-4" />
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={cn(
                      ...
                      {selected === i && <div className="w-2 h-2 rounded-full bg-lime-500" />}
                    </div>
                    <span className="font-heading font-medium text-sm">{t(type.label)}</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-micro text-lime-500/70">Open Telegram →</span>
              </div>
              <p className="text-white font-mono text-lg mt-2">{info.telegram}</p>
              <p className="text-slate-400 text-sm mt-1">Quick response via Telegram</p>
            </a>

            {/* Location */}
            <div className="glass rounded-xl p-5">
              <span className="text-slate-500 text-xl">📍</span>
              <p className="text-white text-sm mt-2 whitespace-pre-line">{t(info.location)}</p>
            </div>

            {/* Legal */}
            <div className="mt-4 px-1">
              <p className="font-mono text-[11px] text-slate-600">{siteConfig.footer.legal.name}</p>
              <p className="font-mono text-[11px] text-slate-600">{siteConfig.footer.legal.bin}</p>
            </div>
          </motion.div>
        </div>
      </div>
